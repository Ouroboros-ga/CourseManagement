"""时间标准化：周次 / 星期 / 节次解析（自遗留参考移植，保留原测试边界）。

来源 ``docs/legacy_parser_reference/normalize.py``，逐条对齐原行为：

- :func:`parse_weeks`：``1-16`` / ``1-15单周`` / ``2-16双周`` / ``1,3,5,8`` /
  ``第1-8周`` 及组合 → 排序去重的周列表；
- :func:`parse_weekday`：``1~7`` 或 ``周一..周日`` / ``星期一..星期日`` → 1..7；
- :func:`parse_period`：小节 → 标准大节折算，含 ``大`` 时直接按大节解析。

非法输入（倒置区间、空结果、越界、疑似公式、无法识别文本）抛 ``ValueError``，
由上层解析编排捕获为 :class:`app.common.parsing.dto.Issue`。
"""
...
def _parse_int_range(part: str, lo: int, hi: int, original: object, label: str) -> list[int]:
    if "-" in part:
        if part.count("-") != 1:
